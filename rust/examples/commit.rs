...
pub async fn main() -> anyhow::Result<()> {
    env_logger::init();
    let mut server = Query::builder().filter("hostname", regexp(".*payment-staging.*"))
        .filter("os", not(empty()))
    ...
    new_sg.set("hostname", "yannik-adminapi-rs-2.test.sg")
        .set("project", "test")
        .add("responsible_admin", "yannik.schwieger")
        .add("protocol_ports_inbound", "tcp443");

    let changes = Commit::new()
        .update(server.changeset())
        .create(new_sg);
