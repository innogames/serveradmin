...
function refresh_servers(callback) {
    ...
    $.getJSON(shell_results_url, search_request, function(data) {
        ...
        $('#understood').text(data['understood']);
...
function regenerate_link() {
    ...
    $('#search_link').attr('href', '?' + $.param(args)).show();
...
function build_server_table(servers, attributes, offset) {
    ...
    for (let i = 0; i < attributes.length; i++) {
        let column = $('<th scope="col">' + attributes[i] +'</th>');
        $('#shell_servers table thead tr:last').append(column);
    }

    let delete_set = {};
    for (let i = 0; i < commit['deleted'].length; i++) {
        delete_set[commit['deleted'][i]] = true;
    }

    // Fill table
    search['no_mapping'] = {};
    let marked_servers = get_marked_servers();
    for (let i = 0; i < servers.length; i++) {
        let server = servers[i];
        let row_class = i & 1 ? 'row_a' : 'row_b';
        ...
        let row = $('<tr></tr>');
        let check = $('<input type="checkbox" name="server"/>')
        ...
        for (let j = 0; j < attributes.length; j++) {
            ...
            let is_editable = search['editable_attributes'][server['servertype']].includes(attr_name);
            let value = server[attr_name];
            ...
            if (typeof changes[server['object_id']] != 'undefined' &&
                ...
                let change = changes[server['object_id']][attr_name];
                if (change['action'] === 'update') {
                    let value_str = format_value(value, attr_name);
                    let new_value_str = format_value(change['new'], attr_name);
                    // TODO: highlight of old value does not match
                    let del_value = $('<del></del>').text(value_str);
                    let ins_value = $('<ins></ins>').text(new_value_str);
                    let table_cell = $('<td></td>').append(del_value)
                    ...
                    let new_value_str = format_value(change['new'], attr_name);
                    let ins_value = $('<ins></ins>').text(new_value_str);
                    let table_cell = $('<td></td>').append(ins_value);
                    _make_attr_editable(table_cell, server, attr_name, change['new']);
                    row.append(table_cell);
                    ...
                    for (let k = 0; k < value.length; k++) {
                        let value_str = format_value(value[k], attr_name, true);
                    ...
                    for (let k = 0; k < change['add'].length; k++) {
                        let value_str = format_value(change['add'][k], attr_name, true);
                ...
                let value_str = format_value(value, attr_name);
                let table_cell = $('<td></td>').text(value_str);
                row.append(table_cell);
        ...
        $('#shell_servers table tbody').append(row);
    ...
    let heading = '<h3>Results (' + search['num_servers'] + ' servers, ';
    heading += 'page ' + search['page'] + '/' + search['num_pages'] + ')</h3>';
    $('#shell_servers').empty().append(heading);
}
...
function render_server_table() {
    ...
    $('#accordion-attributes input[name="attr"]').prop('checked', false);
    for (var i = 0; i < shown_attributes.length; i++) {
        $('#accordion-attributes input[value="' + shown_attributes[i] + '"]').prop('checked', true);
    }
    regenerate_link();
...
function autocomplete_command(term, autocomplete_cb) {
    var autocomplete = [];
...
function handle_command_export() {
    $.get(shell_export_url, {'term': $('#search').val()}, function(hostnames) {
...
$(function() {
    ...
    $('#search').bind('change', function(ev) {
        $('#understood').text('');
        $('#shell_servers').empty();
    });
    ...
    $('#command').autocomplete({
        'source': function(request, response) {
            autocomplete_command($.trim(request.term), response);
        },
        'delay': 0,
        'autoFocus': true
    });

    $('#command').keydown(function(ev) {
    ...
    $('#command_help_icon').click(function() {
        $('#command_help').dialog({
            'width': '70em'
        });
    });

    $('#search_help_icon').click(function() {
        $('#search_help').dialog({
            'width': '70em'
        });
    });
    ...
    $('#accordion-attributes input[name="attr"]').each(function() {
        if (this.checked) {
            search['shown_attributes'].push(this.value);
        }
    });

    $('#accordion-attributes input[name="attr"]').bind('change', function(ev) {
    ...
    $('#accordion-attributes li').each(function() {
        var attr_item = $(this);
    ...
    $('#accordion-attributes li .attr-tooltip').each(function() {
        $(this).tooltip();
    });

    $('#accordion-attributes_toggle').click(function() {
        var attribute_list = $('#accordion-attributes_content').toggle();
        if (attribute_list.is(':visible')) {
            $('#accordion-attributes_toggle').text('(hide)');
        } else {
            $('#accordion-attributes_toggle').text('(show)');
        }

    });

    $('#command').bind('keydown', function(ev) {
