...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--superuser', action='store_true', help='Create superuser token')
        parser.add_argument(
            '--non-interactive',
            action='store_true',
            help=(
    ...
    def handle(self, *args, **options):
        if options['non_interactive']:
            ...
            superuser = bool(environ.get('SERVERADMIN_TOKEN_SUPERUSER', default=False))
        else:
            if options['owner']:
                owner = options['owner']
            else:
        ...
        app = Application.objects.filter(owner=user.get(), name=f'default app for {owner}')
        if app.exists():
            self.stdout.write(self.style.WARNING(f'Default app for {owner} already exists - skipping.'))
            return
